...
class Quaternion:
    ...
    def __init__(self):
        self.origin: np.array = np.array([0., 0., 0.])
        self.axis: np.array = np.array([0, 0, 0])
        self.print_out: bool = True

    # vector is defined as v = p1 - p0
    def set_rotate_axis(self, p0: np.array, p1: np.array) -> np.array:
        q = p1 - p0
    ...
    def rotate_vector(self, p0: np.array, p1: np.array, theta_deg: float) -> typing.Tuple[np.array, np.array]:
        v0 = p0 - self.origin
        v1 = p1 - self.origin
    ...
    def rotate(self, q3: np.array, v3: np.array, angle_deg: float) -> np.array:

        # Construct proper 4-vector for quaternion rotation
        ...
        # Do the quaternion rotation multiplication
        rot_v = self.quaternion_rotate_core(qq, v4)
    ...
    def quaternion_rotate_core(self, qr: np.array, v: np.array):
...
# Testing
###########
# v1 - v0 : vector to be rotated
v1 = np.array([5, 5, 5])
v0 = np.array([2, 0, 1])
# qv - qv0 : rotation axis vector
qv = np.array([3, 0, 8])
qv0 = np.array([1, 0, 3])
# angle of rotation
r_angle_deg = 10
...
for i in range(35):
    r0, r1 = qtr.rotate_vector(v0, v1, r_angle_deg)
